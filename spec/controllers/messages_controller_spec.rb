...
RSpec.describe MessagesController, type: :controller do
  ...
  describe 'POST#Create' do
    ...
    it 'creates a new messages' do
      post :create
      expect(message).to redirect_to(message_url(assigns(:message)))
    end
    ...
    context "When Redering to the template in format JSON and XML" do

      it 'Redering to the templete index in format JSON' do
        get :index, :format => :json
        # return data type JSON in chaset=UTF-8, where equal to type content to response.
        expect(response.content_type).to eq("application/json; charset=utf-8")
      end

      it "Redering to the Index Template the data in format xml " do
    ...
    context "When the message saves successfully" do
      it "sets a flash[:notice] message" do
        post :create # Request type CREATE
